...
/** Default flight path — straight horizontal line through the middle, right → left. */
export const AIRCRAFT_PATH: PathKeyframe[] = [
  { t: 0.0, x: 1.20, y: 0.5 },
  { t: 0.5, x: 0.5, y: 0.5 },
  { t: 1.0, x: -0.20, y: 0.5 },
];
...
export default function AircraftScrollReveal({
  src = "/aircraft.webm",
  height = "20vh",
  className = "",
  path = AIRCRAFT_PATH,
  ...
  const copyPath = useCallback(() => {
    ...
    const code = buildPathExportCode("AIRCRAFT_PATH", sorted);
    navigator.clipboard
      .writeText(code)
      .then(() => {
        setCopied(true);
        console.log("[AircraftPathEditor] Copied:\n\n" + code);
        window.setTimeout(() => setCopied(false), 2500);
      })
      .catch(() => {
        console.log("[AircraftPathEditor] Clipboard unavailable. Code:\n\n" + code);
      });
  }, [editorKeyframes]);
